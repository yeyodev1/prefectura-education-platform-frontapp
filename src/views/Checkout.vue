...
.checkout { width: 100%; padding: 24px 16px; }
.container {
  width: 100%;
  max-width: 1080px;
  margin: 0 auto;
...
@media (min-width: 960px) {
  .container { grid-template-columns: 2fr 1.4fr; }
}
.left { display: grid; gap: 12px; }
.brand img { width: 120px; }
.title { color: $FUDMASTER-DARK; font-size: 24px; margin: 0; }
.subtitle { color: #777; font-size: 14px; margin: 0 0 8px; }
.form { display: grid; gap: 12px; }
.label { font-size: 13px; color: #555; }
.input { display: flex; align-items: center; gap: 10px; border: 1px solid #e6e6e6; border-radius: 10px; padding: 12px 14px; background: $FUDMASTER-LIGHT; }
.input i { color: $FUDMASTER-DARK; }
.input input { flex: 1; border: none; outline: none; background: transparent; color: $FUDMASTER-DARK; font-size: 16px; }
.error { display: flex; align-items: center; gap: 8px; color: $alert-error; background: $alert-error-bg; border: 1px solid rgba($alert-error, 0.3); border-radius: 10px; padding: 10px 12px; font-size: 14px; }
.cta { width: 100%; display: inline-flex; align-items: center; justify-content: center; gap: 10px; background: $FUDMASTER-GREEN; color: $white; border: none; border-radius: 10px; padding: 12px 14px; cursor: pointer; font-weight: 600; font-size: 16px; }
.legal { text-align: center; color: #777; font-size: 12px; }

.right .card { display: grid; gap: 12px; background: $white; border-radius: 16px; padding: 16px; }
.plan { color: $FUDMASTER-DARK; font-weight: 700; font-size: 20px; margin: 0; }
.info { color: #777; font-size: 14px; margin: 0; }
.pricing { display: grid; gap: 6px; }
.row { display: flex; align-items: center; justify-content: space-between; }
.row.small { font-size: 14px; color: #777; }
.price { font-weight: 700; }
.strike { text-decoration: line-through; color: #777; }
.save { color: $FUDMASTER-GREEN; font-weight: 600; }
.timer { background: $FUDMASTER-GREEN; color: $white; border-radius: 10px; padding: 10px 12px; font-weight: 700; }
.benefits .title { color: $FUDMASTER-DARK; font-weight: 700; }
.benefits ul { list-style: none; padding: 0; margin: 0; display: grid; gap: 8px; }
.benefits li { display: flex; align-items: center; gap: 8px; color: #333; }
</style>
